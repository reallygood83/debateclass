...
export async function POST(request: Request) {
  try {
    const { sessionId, terms } = await request.json()
    ...
    return NextResponse.json({ 
      success: true,
      message: '용어가 성공적으로 업데이트되었습니다.'
    })
  } catch (error) {
    console.error('용어 업데이트 오류:', error)
    return NextResponse.json(
      { error: '용어 업데이트에 실패했습니다.' }, 
      { status: 500 }
    )
  }
}